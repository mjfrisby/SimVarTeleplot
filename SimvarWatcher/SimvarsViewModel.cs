    ...
    public class SimvarsViewModel : BaseViewModel, IBaseSimConnectWrapper
    {
        ...
        public SIMCONNECT_SIMOBJECT_TYPE eSimObjectType
        {
            ...
            set
            {
                ...
                this.OnPropertyChanged("lObjectIDs");
                bObjectIDSelectionEnabled = (m_eSimObjectType != SIMCONNECT_SIMOBJECT_TYPE.USER);
                if (!alltypevalue[m_eSimObjectType].Contains(iObjectIdRequest))
                {
                    if (alltypevalue[m_eSimObjectType].Count > 0)
                        iObjectIdRequest = alltypevalue[m_eSimObjectType][0];
                    else
                        iObjectIdRequest = uint.MaxValue;
                }
        ...
        Dictionary<SIMCONNECT_SIMOBJECT_TYPE, ObservableCollection<uint>> alltypevalue;
        ...
        public SimvarsViewModel()
        {
            ...
            alltypevalue = new Dictionary<SIMCONNECT_SIMOBJECT_TYPE, ObservableCollection<uint>>();
            alltypevalue[SIMCONNECT_SIMOBJECT_TYPE.USER] = new ObservableCollection<uint>();
            alltypevalue[SIMCONNECT_SIMOBJECT_TYPE.USER].Add(0); // Magic number for the User aircraft
            alltypevalue[SIMCONNECT_SIMOBJECT_TYPE.ALL] = new ObservableCollection<uint>();
            alltypevalue[SIMCONNECT_SIMOBJECT_TYPE.AIRCRAFT] = new ObservableCollection<uint>();
            alltypevalue[SIMCONNECT_SIMOBJECT_TYPE.HELICOPTER] = new ObservableCollection<uint>();
            alltypevalue[SIMCONNECT_SIMOBJECT_TYPE.BOAT] = new ObservableCollection<uint>();
            alltypevalue[SIMCONNECT_SIMOBJECT_TYPE.GROUND] = new ObservableCollection<uint>();
        ...
        private void SimConnect_OnRecvSimobjectDataBytype(SimConnect sender, SIMCONNECT_RECV_SIMOBJECT_DATA_BYTYPE data)
        {
            ...
            if (!alltypevalue[eSimObjectType].Contains(iObject))
            {
                alltypevalue[eSimObjectType].Add(iObject);
            }
            lObjectIDs = alltypevalue[m_eSimObjectType];
            this.OnPropertyChanged("lObjectIDs");
            foreach (SimvarRequest oSimvarRequest in lSimvarRequests)
            {
                if (iRequest == (uint)oSimvarRequest.eRequest && (m_iObjectIdRequest == 0 || iObject == m_iObjectIdRequest))
                {
